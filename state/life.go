...
func isAlive(backend modelBackend, collName string, id interface{}) (bool, error) {
	coll, closer := backend.db().GetCollection(collName)
	defer closer()
...
func isNotDead(backend modelBackend, collName string, id interface{}) (bool, error) {
	coll, closer := backend.db().GetCollection(collName)
	defer closer()
