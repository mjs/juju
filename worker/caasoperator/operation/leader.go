...
func (rl *resignLeadership) Execute(state State) (*State, error) {
	...
	//
	// So. I *think* that the right approach is to implement a no-api caasoperator
	// variant, that we run *instead of* the normal caasoperator when the API is
	// unavailable, and replace with a real caasoperator when appropriate; this
	// implies that we need to take care not to allow the implementations to
	// diverge, but implementing them both as "caasoperators" is probably the best
