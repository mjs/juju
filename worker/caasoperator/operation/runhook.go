...
func (rh *runHook) Execute(state State) (*State, error) {
	...
	rh.runner.CAASContext().ResetExecutionSetApplicationStatus()
	...
	default:
		...
		rh.callbacks.NotifyHookFailed(rh.name, rh.runner.CAASContext())
		return nil, ErrHookFailed
	}

	if ranHook {
		logger.Infof("ran %q hook", rh.name)
		rh.callbacks.NotifyHookCompleted(rh.name, rh.runner.CAASContext())
	} else {
...
func (rh *runHook) beforeHook() error {
	...
	switch rh.info.Kind {
	case hooks.Install:
		err = rh.runner.CAASContext().SetApplicationStatus(jujuc.StatusInfo{
...
func (rh *runHook) afterHook(state State) (bool, error) {
	ctx := rh.runner.CAASContext()
	hasRunStatusSet := ctx.HasExecutionSetApplicationStatus() || state.StatusSet
	var err error
	switch rh.info.Kind {
	case hooks.Stop:
	...
	case hooks.Start:
		...
		logger.Debugf("application %v has started but has not yet set status", ctx.ApplicationName())
