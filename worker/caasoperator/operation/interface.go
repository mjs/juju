...
// Operation encapsulates the stages of the various things the caasoperator can do,
...
// Executor records and exposes caasoperator state, and applies suitable changes as
...
// Callbacks exposes all the caasoperator code that's required by the various operations.
...
type Callbacks interface {
	...
	// to untangle CaasOperator.relationers from everything else. They're only used by
	// RunHook operations.
	...
	NotifyHookCompleted(string, runner.CAASContext)
	NotifyHookFailed(string, runner.CAASContext)
